...
{
	dlistint_t *coup = NULL, *new_node = NULL;
	...
	while (coup != NULL)
	{
		if (iter_times == idx)
		{
			new_node = create_node(n, coup, coup->prev);
			coup->prev = new_node;
			coup = new_node;
			coup->prev->next = new_node;
			return (new_node);
		}
...
/**
  * dlistint_len - Counts the number  elements in doubly linked list
